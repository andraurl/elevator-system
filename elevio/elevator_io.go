...
// MakeButtonEvent return ButtonEvent given buttonNr and floorNr
func MakeButtonEvent(buttonNr int, floorNr int) ButtonEvent {
	//println("MakeButtonEvent:  ", buttonNr, " ", floorNr)
	order := ButtonEvent{
		Button: ButtonType(buttonNr),
		Floor:  floorNr,
	}
	return order
}
...
func turnOffAllLights() {
	for i := 0; i < config.NumFloors; i++ {
		for k := 0; k < config.NumButtonTypes; k++ {
			button := MakeButtonEvent(k, i)
			SetButtonLamp(button.Button, button.Floor, false)
		}
	}
}
